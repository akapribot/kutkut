...
import org.alicebot.server.core.targeting.gui.CategoryPanel;
import org.alicebot.server.core.targeting.gui.InputPanel;
    ...
    implements Runnable
{
    ...
    static void add(Target target, HashMap hashmap)
    {
        ...
        if(hashmap == savedTargets)
            save(target);
        else
        if(hashmap == discardedTargets)
            discard(target);
        updatePanels();
    }
