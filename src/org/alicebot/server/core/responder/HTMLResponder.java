...
public class HTMLResponder extends AbstractMarkupResponder
{
    ...
    public String authenticate(HttpServletRequest httpservletrequest, HttpServletResponse httpservletresponse, String s)
    {
        ...
        if(parameterEquals(httpservletrequest, "change-password", "yes"))
        {
            i |= 0x20;
            i |= 1;
        } else
        if(parameterEquals(httpservletrequest, "checkauth", "auth"))
        {
            i |= 8;
            i |= 1;
        } else
        if(parameterEquals(httpservletrequest, "register", "yes"))
        {
            i |= 0x10;
            i |= 4;
        } else
        if(parameterEquals(httpservletrequest, "register", "auth"))
        {
            i |= 0x10;
            i |= 1;
        } else
