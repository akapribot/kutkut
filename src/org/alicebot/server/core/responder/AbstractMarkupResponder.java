    ...
    public String preprocess(String s, String s1)
    {
        ...
        for(int j = 0; j < i; j++)
        {
            ...
            if(obj instanceof Tag)
            {
                ...
                if(s2.equals("userinput/"))
                    response.append(s);
                else
                if(s2.startsWith("bot name=\""))
                {
                ...
                if(s2.equals("alice_in"))
                    response.append(s);
                else
                if(s2.startsWith("bot_"))
                {
    ...
    public String append(String s, String s1, String s2)
    {
        ...
        for(int j = 0; j < i; j++)
        {
            ...
            if(obj instanceof Tag)
            {
                ...
                if(s3.equals("response/"))
                {
                    if(response.length() > 0)
                        response.append(' ');
                    response.append(s1);
                } else
                if(s3.startsWith("bot name=\""))
                {
                    int k = s3.lastIndexOf("\"");
                    if(k > 10)
                        response.append(bot.getPropertyValue(s3.substring(10, k)));
                } else
                if(s3.equals("alice_in"))
                    response.append(s);
                else
                if(s3.equals("alice_out"))
                {
                    if(response.length() > 0)
                        response.append(' ');
                    response.append(s1);
                } else
                if(s3.startsWith("bot_"))
                {
