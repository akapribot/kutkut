...
//                    catch(InterruptedException interruptedexception)
            ...
            if(s.indexOf('/') == 0)
            {
                ...
                if(s.toLowerCase().startsWith("/load"))
                    load(s, botid);
                else
                if(s.toLowerCase().startsWith("/unload"))
                    unload(s, botid);
                else
                if(s.toLowerCase().equals("/bots"))
                    showBotList();
                else
                if(s.toLowerCase().startsWith("/talkto"))
                    talkto(s);
                else
                if(s.toLowerCase().equals("/who"))
                    who();
                else
                if(s.toLowerCase().equals("/files"))
                    listBotFiles();
                else
                if(s.toLowerCase().startsWith("/roll chatlog"))
                    rollChatLog(botid);
                else
                if(s.toLowerCase().equals("/roll targets"))
                    rollTargets();
                else
                if(s.toLowerCase().equals("/commandables"))
                    listCommandables();
                else
                    try
                    {
